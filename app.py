...
### ONE HOT ENCODING JUST IN CASE

# one_hot_encoder = OneHotEncoder(handle_unknown='ignore', sparse=False)
#
#         catagorical_columns = ["smoking_status", 'work_type']
#         numerical_columns = ["age", "hypertension", "heart_disease", "avg_glucose_level", "bmi", 'age/bmi']
#         # ONE HOT ENCODE CATAGORICAL COLUMNS
#         OH_columns_df = pd.DataFrame(one_hot_encoder.fit_transform(df[catagorical_columns]))
#         st.write("OH_columns_df")
#         OH_columns_df.columns = one_hot_encoder.get_feature_names_out(catagorical_columns)
#
#         OH_columns_df.reset_index(drop=True, inplace=True)
#
#         # DROP CATAGORICAL COLUMNS FROM X_train AND REPLACE
#         num_X_df = df.drop(catagorical_columns, axis=1)
#
#         num_X_df.reset_index(drop=True, inplace=True)
#
#         OH_X_df = pd.concat([num_X_df, OH_columns_df], axis=1)
#         st.write(OH_X_df.head())
